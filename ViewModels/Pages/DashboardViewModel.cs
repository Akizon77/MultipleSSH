...
namespace MultipleSSH.ViewModels.Pages
{
    ...
    {
        [ObservableProperty]
        string fastHost = String.Empty;
        [ObservableProperty]
        string fastPort = "22";
        [ObservableProperty]
        string fastUsername = String.Empty;
        [ObservableProperty]
        string fastPwd = String.Empty;
        [ObservableProperty]
        string fastKeyPath = String.Empty;
        [ObservableProperty]
        //0 密码；1 私钥
        int fastVerigyMethod = 0;
        [ObservableProperty]
        ObservableCollection<Models.SshHost> _hosts = new(AppSettings.Instance.Hosts);
        [RelayCommand]
        async Task OnFastConnect()
        {
            await OnConnect(new Models.SshHost()
            {
                Host = FastHost,
                Port = FastPort,
        ...
        [RelayCommand]
        async Task OnConnect(object o)
        {
            var host = o as Models.SshHost;
            if (String.IsNullOrEmpty(host.Host)) host.Host = "example.com";
            if (String.IsNullOrEmpty(host.Port)) host.Port = "22";
            if (String.IsNullOrEmpty(host.Username)) host.Username = "root";
            ProcessStartInfo psi = new();
            psi.UseShellExecute = true;
            psi.FileName = "cmd.exe";
            if (host.LoginMethod == LoginMethod.PrivateKey)
            {
                psi.Arguments = $"/c ssh {host.Username}@{host.Host} -p {host.Port} -i {host.PrivateKey} & pause";
                Process.Start(psi);
                return;
            }
            ...
            if (messageBoxResult == Wpf.Ui.Controls.MessageBoxResult.None) return;
